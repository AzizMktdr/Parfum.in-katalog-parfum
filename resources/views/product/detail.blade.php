...
<div class="detail-page">
    ...
    <div class="detail-main-grid">
        ...
        <div class="detail-image-wrap">
            <img src="{{ $product->image_url }}"
                 alt="{{ $product->name }}"
                 onerror="this.src='{{ asset('images/products/california-signature.png') }}'">
        </div>
    ...
    <hr class="detail-divider">
    {{-- Accords section --}}
    <div class="detail-accords-section">
        <div class="detail-desc-title">Notes</div>
        <p class="detail-desc-subtitle" style="margin-bottom:20px;">Grouped by Accord</p>
        <div class="detail-accords-grid">
            @foreach($product->accords as $accord)
            <div class="detail-accord-block" style="border-left: 3px solid {{ $accord->color ?? '#888' }};">
                <span class="detail-accord-block-name" style="color: {{ $accord->color ?? 'var(--text)' }}">{{ $accord->name }}</span>
                @if($accord->description)
                <p class="detail-accord-block-desc">{{ $accord->description }}</p>
                @endif
                {{-- Notes yang termasuk accord ini --}}
                @php
                    $allNotes = $product->topNotes->merge($product->middleNotes)->merge($product->baseNotes);
                @endphp
                @if($allNotes->isNotEmpty())
                <div class="detail-accord-notes">
                    @foreach($allNotes->take(4) as $note)
                    <span class="detail-accord-note-chip">{{ $note->name }}</span>
                    @endforeach
                </div>
                @endif
            </div>
        ...
        <div class="product-grid">
            ...
            <a href="{{ route('product.detail', $item['slug']) }}" class="product-card">
                <div class="product-card-img">
                    <img src="{{ $item['image'] ? asset($item['image']) : '' }}"
                         alt="{{ $item['name'] }}"
