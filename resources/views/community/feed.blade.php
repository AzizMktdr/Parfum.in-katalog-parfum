...
<div class="community-page">
    ...
    <div class="community-header">
        ...
        <div class="community-title-divider">
            <span></span>
    ...
    <div class="feed-tabs">
        <a href="{{ route('community') }}?tab=discussions"
           class="feed-tab {{ $tab === 'discussions' ? 'active' : '' }}">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
            Diskusi
        </a>
        <a href="{{ route('community') }}?tab=collections"
           class="feed-tab {{ $tab === 'collections' ? 'active' : '' }}">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 2h6M10 2v4.2c0 .5-.15 1-.45 1.4L6.6 11.8c-.6.8-.9 1.7-.9 2.7v4.5a3 3 0 0 0 3 3h6.6a3 3 0 0 0 3-3v-4.5c0-1-.3-1.9-.9-2.7l-2.95-4.2A2.4 2.4 0 0 1 14 6.2V2"/></svg>
            Koleksi
        </a>
    </div>
        ...
        <div class="feed-list">
            ...
            <div class="discussion-card" id="discussion-{{ $d->id }}">
                ...
                <div class="feed-card-header">
                    ...
                    <div class="feed-card-menu">
                        <button type="button" class="feed-card-menu-btn" data-target="menu-{{ $d->id }}">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="19" cy="12" r="2"/></svg>
                        </button>
                        <div class="feed-card-menu-dropdown" id="menu-{{ $d->id }}" style="display:none">
                            <form method="POST" action="{{ route('discussion.destroy', $d) }}" onsubmit="return confirm('Hapus diskusi ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="feed-card-menu-delete">Hapus Diskusi</button>
                            </form>
                        </div>
                    </div>
                    @endif
...
document.querySelectorAll('.feed-card-menu-btn[data-target]').forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const menu = document.getElementById(btn.dataset.target);
        if (!menu) return;
        const isHidden = menu.style.display === 'none';
        document.querySelectorAll('.feed-card-menu-dropdown').forEach(m => m.style.display = 'none');
